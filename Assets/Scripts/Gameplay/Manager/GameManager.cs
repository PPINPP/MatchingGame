...
public class GameManager : SingletonSerializedMonobehaviour<GameManager>
{
    ...
    {
        var randomedCard = Utils.GetRndCardFromTargetAmount(_targetPairMatchCount, gameDifficult, GameplayResources.Instance.CardCategoryDataDic[categoryTheme]);
        var cardPropList = Utils.CreateCardPair(gameDifficult, randomedCard);
        cardPropList = new List<CardProperty>(Utils.ShuffleCard(cardPropList,pairConfig.roundShuffle));
